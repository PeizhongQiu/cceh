...
Segment * Segment_Split(Segment *seg)
{
    ...
    u64 pattern = ((size_t)1 << (sizeof(Key_t)*8 - seg->local_depth - 1));
...
int insert_hash(HASH *dir, Key_t new_key, Value_t new_value)
{
    ...
    u64 pattern = (key_hash >> (8*sizeof(key_hash) - seg->local_depth));
    unsigned i;
    for (i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i)
    {
        ...
        if (seg->_[slot].key == INVALID ||
            (hash_64(seg->_[slot].key) >> (key_size - seg->local_depth)) != pattern)
        {
